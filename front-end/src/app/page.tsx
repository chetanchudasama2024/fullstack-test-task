...
export default function Menu() {
  ...
  const getMenu = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/menu`, { method: "GET" });
      if (res.ok) {
  ...
  const deleteMenuById = async (menuId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        dispatch(
          setData(getData.filter((item) => !idsToDelete.includes(item.menuId)))
        );
            ...
            <div key={subItem.menuId} className="flex py-1">
              ...
              <div className="flex flex-col">
                ...
                {(isAllExpanded || activeSubMenu === subItem.menuId) && (
                  <SubMenuItem
                    parentId={subItem.menuId}
                    depth={depth}
                    parentLabel={subItem.name}
                  />
                )}
              </div>
            </div>
  ...
  return (
    <div className="block sm:flex">
      ...
      <div
        className={`w-60 bg-gray-800 h-[100vh]   sm:${
      ...
      <div className="w-calc([100%-w-60])">
        <img
          className="w-6 h-6 block ml-12 mt-5 sm:hidden"
          src="/icons/menu.svg"
        ...
        <div className="block ml-12 sm:flex">
          ...
          <div className="relative h-[454px] mt-14 left-0  sm:w-[532px] sm:left-52">
            ...
            <button
              onClick={() =>
                handleSave(menuId, depth, parentIndex, subMenuName)
              }
